...
We first print the output (saved in .csv files in the repository) for easy viewing. Then, we display the code that generates the output. Warning, this code below is computational intensive. It is advised that users run the code in Section 3, which is an example using a single set of parameters.
...
## iterate over different effect sizes
for (real_bias_es in c(es_outcome_smaller, es_outcome_larger)) {
