...
Warning, the code below will zap cpu! best to run the code above which is an example using a single set of parameters. The output (saved in .csv files in the repository) are called first for easy viewing.
...
read_csv("pwr-rqs-codebook.csv") %>%
  ...
  slice(match(header, c("n_ids", "n_countries", "bias_es", "beta",	"random",	"trials",	"successes", "mean",	"lower", "upper",	"powered", "Class"))) %>%
...
read_csv("pwr-rq1-sims-combined.csv") %>%
  arrange(bias_es, random, desc(n_countries), desc(n_ids)) %>%
  mutate(powered = if_else(mean > 0.95, "yes", "no")) %>%
    select(n_ids, n_countries, bias_es,	beta, random,	trials, successes, mean, lower, upper,	powered) %>%
...
Here is the iterative code for RQ1.
...
Here is the iterative code for RQ2.
...
Here is the iterative code for RQ3.
